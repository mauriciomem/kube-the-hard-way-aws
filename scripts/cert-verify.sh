...
WORKER_1_CERT=/var/lib/kubelet/k8s-worker-1.crt
WORKER_1_KEY=/var/lib/kubelet/k8s-worker-1.key
...
# Systemd k8s-worker-1 kubelet location
SYSTEMD_WORKER_1_KUBELET=/etc/systemd/system/kubelet.service

# kube-proxy k8s-worker-1 location
...
echo "This script will validate the certificates in master as well as k8s-worker-1 nodes. Before proceeding, make sure you ssh into the respective node [ Master or Worker-1 ] for certificate validation"
echo
...
echo "  4. Verify kubeconfigs and PKI on k8s-worker-1 Node after step 10"
echo "  5. Verify kubeconfigs and PKI on k8s-worker-2 Node after step 11"
echo
...
case $value in
  ...
  1)
    if ! [ "${HOST}" = "k8s-master-1" -o "${HOST}" = "k8s-master-2" ]
    then
        printf "${FAILED}Must run on k8s-master-1 or k8s-master-2${NC}\n"
        exit 1
    fi
    ...
    if [ "${HOST}" = "k8s-master-1" ]
    then
        #check_cert_and_key "admin" $SUBJ_ADMIN $CERT_ISSUER    # The administrator user will be in a client node, not the master node
  ...
  4)
    if ! [ "${HOST}" = "k8s-worker-1" ]
    then
        printf "${FAILED}Must run on k8s-worker-1${NC}\n"
        exit 1
    fi
    ...
    check_cert_only "ca" $SUBJ_CA $CERT_ISSUER
    check_cert_and_key "kube-proxy" $SUBJ_KP $CERT_ISSUER
    check_cert_and_key "k8s-worker-1" "Subject:CN=system:node:k8s-worker-1,O=system:nodes" $CERT_ISSUER
  ...
  5)
    if ! [ "${HOST}" = "k8s-worker-2" ]
    then
        printf "${FAILED}Must run on k8s-worker-2${NC}\n"
        exit 1
    fi
    ...
    check_cert_only "kubelet-client-current" "Subject:O=system:nodes,CN=system:node:k8s-worker-2" $CERT_ISSUER
    check_kubeconfig "kube-proxy" "/var/lib/kube-proxy" "https://${LOADBALANCER}:6443"
    ;;
